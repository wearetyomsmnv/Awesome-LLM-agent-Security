...
| [IronClaw](https://github.com/IronSecCo/ironclaw) | Agent Sandbox & Gateway | Security-first, self-hosted platform for running AI agents in isolation | - Per-agent gVisor (`runsc`) sandbox with `network=none`<br>- Human approval gateway, deny-by-default<br>- Per-session encrypted (SQLCipher) queues<br>- Signed/attested supply chain (cosign, SBOM, build provenance) |
</div>
