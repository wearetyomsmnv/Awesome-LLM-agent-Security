...
| [TWZRD Agent Intel](https://intel.twzrd.xyz) | Identity Verification MCP | Trust scoring for AI agent wallets on Solana. Verify counterparty identity before x402 micropayments. | - `score_agent(wallet)` — reputation score<br>- `preflight_check(wallet)` — pre-payment safety check<br>- `get_trust_receipt(wallet)` — verifiable trust proof (x402 paid)<br>- Free streamable-http MCP endpoint |
