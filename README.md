...
| [agentsurface](https://github.com/Northbeams-Labs/agentsurface) | Inventory Tool | Local inventory of the AI agent machinery installed on a machine | - MCP servers across many clients, plus extensions, plugins, skills, connectors, scheduled tasks and instruction files<br>- Reports what each item declares it can reach, with the file path<br>- Drift diff against a local hash baseline<br>- No network calls, no account, no upload; enforced by a CI check<br>- Inventory only: no risk score, no malice judgement, no prompt-injection detection |
